...
# --- CLASE PDF CON SU NOMBRE AL PIE ---
class PDF(FPDF):
    def footer(self):
        self.set_y(-15)
...
# --- 5. FINANZAS (CON PRESUPUESTO Y BORRADO) ---
if menu == "💰 FINANZAS":
    ...
    c1, c2 = st.columns(2)
    with c1: st.markdown(f"<div class='balance-box'><h3>DISPONIBLE</h3><h1 style='color:#2ecc71;'>RD$ {df_f['monto'].sum() if not df_f.empty else 0:,.2f}</h1></div>", unsafe_allow_html=True)
    ...
    if presupuesto_mensual > 0:
        st.write(f"**Uso Presupuesto:** {gastos_mes:,.2f} / {presupuesto_mensual:,.2f}")
...
# --- 6. SALUD (BOTONES EN PARALELO - CORREGIDO) ---
elif menu == "🩺 SALUD":
    ...
    with t1:
        ...
        # --- ESTO ES LO QUE USTED PIDÓ: BOTONES AL LADO ---
        ...
        with col_wa:
            if not df_g.empty:
                ...
                st.markdown(f"<div class='btn-whatsapp-paralelo'><a href='https://wa.me/?text={urllib.parse.quote(texto_w)}' target='_blank'>📲 WHATSAPP RÁPIDO</a></div>", unsafe_allow_html=True)
...
# --- 7. BITÁCORA ---
    ...
    if st.button("GUARDAR"):
        ...
        st.success("Guardado.")
    try:
        with open("nexus_notas.txt", "r", encoding="utf-8") as f: st.text_area("Notas:", f.read(), height=300)
