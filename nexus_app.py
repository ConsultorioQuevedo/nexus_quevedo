...
# --- 6. SALUD (CON MENÚ DESPLEGABLE RESTAURADO) ---
    ...
    with t1:
        ...
        if not df_g.empty:
            ...
            st.plotly_chart(px.line(df_g.iloc[::-1], x='fecha', y='valor', markers=True, title="TENDENCIA", template="plotly_dark"))
    ...
    with t2:
        ...
        for _, r in df_meds.iterrows():
            ...
            col_a.markdown(f"<div class='info-box'><b>{r['nombre']}</b> - {r['dosis']} ({r['horario']})</div>", unsafe_allow_html=True)
    ...
    with t3:
        ...
        with st.form("f_citas"):
            ...
            if st.form_submit_button("AGENDAR"):
                db.execute("INSERT INTO citas (doctor, fecha, motivo) VALUES (?,?,?)", (doc, str(fec), "CONSULTA"))
                db.commit(); st.rerun()
        
        df_c = pd.read_sql_query("SELECT id, doctor, fecha FROM citas ORDER BY fecha ASC", db)
