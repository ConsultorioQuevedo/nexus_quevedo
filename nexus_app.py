...
def init_db():
    # Tablas basadas en los diagramas de Luis Rafael Quevedo
...
with st.sidebar:
    ...
    st.metric("Balance Neto Total", f"${calcular_balance():,.2f}")
    st.divider()
    ...
    st.caption("Luis Rafael Quevedo & Gemini AI")
...
if menu == "Dashboard":
    ...
    if data_g:
        ...
        st.plotly_chart(px.line(df_g, x="Fecha", y="Valor", markers=True), use_container_width=True)
...
# --- 5. MÓDULO: FINANZAS ---
    ...
    with f1:
        ...
        with col_in:
            ...
            if st.button("Registrar Movimiento"):
                ...
                st.rerun()
        with col_list:
            st.write("**Historial (Gestionar/Eliminar)**")
    ...
    with f2:
        ...
        p_lim = st.number_input("Límite", min_value=0.0)
        ...
        if lim_act:
            st.write(f"**Uso:** ${gasto_cat} / ${lim_act[0][0]}")
...
# --- 7. MÓDULO: MOTOR DE IA ---
    ...
    st.camera_input("📷 Escanear Gaceno / Receta")
    if st.button("📄 Generar Reporte PDF"):
        st.info("Generando reporte basado en datos soberanos...")

# --- PIE DE PÁGINA ---
st.markdown("---")
st.markdown("<center><b>Sistema Quevedo</b> | Diseñado por: <i>Luis Rafael Quevedo & Gemini AI</i></center>", unsafe_allow_html=True)
