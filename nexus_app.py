...
if menu == "💰 PRESUPUESTO & FINANZAS":
    ...
    # Cargar presupuesto configurado
    ...
    with st.form("registro_fin", clear_on_submit=True):
        ...
        cat = st.text_input("Categoría (Ej: Supermercado, Farmacia)").upper()
...
# --- 6. MÓDULO: CONTROL DE SALUD ---
    ...
    with tab_g:
        ...
        if not df_g.empty:
            ...
            pdf = PDF()
            pdf.add_page()
            pdf.set_font("Arial", 'B', 14); pdf.cell(190, 10, "REPORTE GLUCOSA", ln=True, align='C')
            for _, r in df_g.iterrows(): pdf.cell(190, 8, f"{r['fecha']} {r['hora']} - {r['momento']}: {r['valor']} mg/dL", ln=True)
            st.download_button("📥 DESCARGAR REPORTE PDF", pdf.output(dest='S').encode('latin-1'), "Salud.pdf")
...
# --- 7. MÓDULO: BITÁCORA ---
    ...
    nota = st.text_area("Escribir pensamiento o nota del día:", height=200)
    if st.button("GUARDAR NOTA"):
        with open("nexus_notas.txt", "a", encoding="utf-8") as f:
            f.write(f"[{f_str} {h_str}]: {nota}\n\n")
        st.success("Nota guardada en el archivo local.")
    
    st.markdown("---")
    try:
        with open("nexus_notas.txt", "r", encoding="utf-8") as f:
            st.text_area("Historial completo:", f.read(), height=300)
