...
st.markdown("""
    ...
    .report-table { background-color: white; color: black; padding: 20px; border-radius: 10px; }
    </style>
...
# --- 5. FINANZAS (GRÁFICO DE TARTA GARANTIZADO) ---
if menu == "💰 FINANZAS":
    ...
    with st.form("f_fin", clear_on_submit=True):
        ...
        cat = st.text_input("CATEGORÍA (Ej: COMIDA, FARMACIA, RENTA):").upper()
    ...
    if not df_f.empty:
        ...
        c_m1.markdown(f"<div class='balance-box'><h3>DISPONIBLE</h3><h1 style='color:#2ecc71;'>RD$ {total_disp:,.2f}</h1></div>", unsafe_allow_html=True)
        
        # Termómetro
        porcentaje = min(gastos_mes / presupuesto_mensual, 1.0)
        c_m2.markdown(f"<h3>Presupuesto: {porcentaje*100:.0%}$</h3>", unsafe_allow_html=True)
        c_m2.progress(porcentaje)
        c_m2.write(f"Gastado este mes: RD$ {gastos_mes:,.2f} de RD$ {presupuesto_mensual:,.2f}")

        st.divider()
        
        # EL GRÁFICO DE TARTA
        st.subheader("📊 Distribución de Gastos (Gráfico de Tarta)")
        df_gastos = df_f[df_f['tipo'] == 'GASTO']
        if not df_gastos.empty:
            fig = px.pie(df_gastos, values=abs(df_gastos['monto']), names='categoria', 
                         title="¿En qué se va el dinero?", hole=.4, template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Registre su primer GASTO para ver el gráfico de tarta.")

# --- 6. SALUD (GENERADOR DE REPORTE PARA MÉDICO) ---
elif menu == "🩺 SALUD":
    ...
    with t1:
        ...
        # Indicador de Tendencia (Flechas)
        if len(df_g) >= 2:
            ultimo = df_g.iloc[0]['valor']
            anterior = df_g.iloc[1:6]['valor'].mean()
            if ultimo < anterior - 5:
                st.markdown("<div class='tendencia-box' style='background-color:#166534;'>🟢 TENDENCIA: MEJORANDO (Hacia abajo)</div>", unsafe_allow_html=True)
            elif ultimo > anterior + 5:
                st.markdown("<div class='tendencia-box' style='background-color:#991b1b;'>🔴 TENDENCIA: ALERTA (Subiendo)</div>", unsafe_allow_html=True)
            else:
        ...
        # BOTÓN DE REPORTE (Sustituye al PDF complejo por algo más práctico)
        if st.button("📄 GENERAR REPORTE PARA EL MÉDICO"):
            st.markdown("### 📋 REPORTE OFICIAL DE SALUD - NEXUS")
            st.write("Dígale al médico o imprima esta pantalla:")
            reporte = df_g[['fecha', 'momento', 'valor']].head(20)
            st.dataframe(reporte, use_container_width=True)
            st.success("💡 Consejo: Use 'Ctrl+P' en su teclado para guardar esta tabla como PDF.")

        st.plotly_chart(px.line(df_g.iloc[::-1], x='fecha', y='valor', title="Evolución", template="plotly_dark"))
...
# --- 8. CONFIGURACIÓN ---
    ...
    nuevo_p = st.number_input("Establecer Presupuesto Mensual (RD$):", min_value=0.0, value=20000.0)
