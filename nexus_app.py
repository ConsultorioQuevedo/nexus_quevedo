...
# --- 6. PANTALLA: INICIO (GENERADOR DE ALERTAS) ---
if opcion == "🏠 INICIO":
    ...
    with col_b:
        ...
        if not encontrada: st.success("No tienes citas próximas.")
    ...
    with col_c:
        ...
        else: st.warning("Configura tu presupuesto en Ajustes.")
...
# --- 7. PANTALLA: FINANZAS ---
    ...
    with st.form("form_finanzas", clear_on_submit=True):
        ...
        if st.form_submit_button("REGISTRAR"):
            m_f = -monto if tipo == "GASTO" else monto
            db.execute("INSERT INTO finanzas (fecha, mes, tipo, categoria, detalle, monto) VALUES (?,?,?,?,?,?)", 
                       (fecha_mov.strftime("%d/%m/%Y"), fecha_mov.strftime("%m-%Y"), tipo, cate, deta, m_f))
    ...
    if not df_f.empty:
        ...
        if st.button("🗑️ ELIMINAR ÚLTIMO REGISTRO"):
            db.execute("DELETE FROM finanzas WHERE id = (SELECT MAX(id) FROM finanzas)"); db_conn.commit(); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

# --- 8. PANTALLA: SALUD (WHATSAPP Y BORRADO) ---
elif opcion == "🩺 SALUD":
    st.title("Control de Glucosa")
    df_g = pd.read_sql_query("SELECT * FROM glucosa ORDER BY id DESC", db_conn)
    if not df_g.empty:
        col_p1, col_p2 = st.columns(2)
        with col_p1: st.download_button("📥 REPORTE PDF", exportar_pdf_salud(df_g), f"Salud_{f_txt}.pdf")
        with col_p2:
            u = df_g.iloc[0]
            msg = f"Glucosa Quevedo: *{u['valor']} mg/dL* ({u['momento']}) el {u['fecha']}"
            st.link_button("📲 ENVIAR POR WHATSAPP", f"https://wa.me/?text={urllib.parse.quote(msg)}")
        ...
        if st.button("🗑️ ELIMINAR ÚLTIMA LECTURA"):
            db.execute("DELETE FROM glucosa WHERE id = (SELECT MAX(id) FROM glucosa)"); db_conn.commit(); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
...
# --- 9. MEDICINAS, CITAS, BITÁCORA Y CONFIG ---
    ...
    if not df_c.empty:
        ...
        if st.button("🗑️ BORRAR ÚLTIMA CITA"):
            db.execute("DELETE FROM citas WHERE id = (SELECT MAX(id) FROM citas)"); db_conn.commit(); st.rerun()
    with st.form("f_c"):
    ...
    st.title("Bitácora")
    arch = "nexus_notas.txt"
    nueva = st.text_area("Nota:")
    if st.button("💾 GUARDAR"):
        with open(arch, "a", encoding="utf-8") as f: f.write(f"[{f_txt}]: {nueva}\n\n")
        st.rerun()
    if os.path.exists(arch):
        st.text_area("Historial:", open(arch, "r", encoding="utf-8").read(), height=300)
        if st.button("🗑️ LIMPIAR"): os.remove(arch); st.rerun()

elif opcion == "⚙️ CONFIG":
    st.title("Configuración")
    nuevo_p = st.number_input("Presupuesto RD$:", value=float(presupuesto_val))
...
st.markdown("<p style='text-align: center;'>NEXUS QUEVEDO PRO v5.0 - FIRME</p>", unsafe_allow_html=True)
